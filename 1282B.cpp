...
int check(int start, vector<int> &v, int p, int jump) {
    int n = v.size(), i = start;
    for(; i < n && v[i] <= p; i += jump) {
        p -= v[i];
    }
...
int main() {
    ...
    while(t--) {
        ...
        int ans = 0;
        for(int i = 0, m = p; i < k && m >= v[i]; i++) {
            m -= v[i];
            ans = max(ans, check(i + k, v, m, k));
        }
        ans = max(ans, check(k - 1, v, p, k));
