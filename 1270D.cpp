...
int main() {
    ...
    int first = mp.begin()->first, first_cnt = mp.begin()->second;
    int second = next(mp.begin())->first, second_cnt = next(mp.begin())->second;

    cout << "! " << (first < second ? second_cnt : first_cnt) << endl;
}
