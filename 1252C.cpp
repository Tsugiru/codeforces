...
int main() {
    ...
    while(q--) {
        ...
        int min_r = min(ra, rb), max_r = max(ra, rb);
        int min_c = min(ca, cb), max_c = max(ca, cb);
        int r_odds = br[max_r] - br[min_r - 1];
        int c_odds = bc[max_c] - bc[min_c - 1];

        if ((r_odds == max_r - min_r + 1 && c_odds == max_c - min_c + 1) 
        || (r_odds == 0 && c_odds == 0)) cout << "YES" << endl;
        else cout << "NO" << endl;
    } 
