...
int main() {
    ...
    while(q--) {
        ...
        int top = min(mp['U'], mp['D']);
        int bot = min(mp['R'], mp['L']);
        if(!top && !bot) { cout << 0 << '\n' << "" << endl; continue; }
        string res = top ? bot ? "" : "DU" : "RL";
        for(int i = 0; i < bot && top; i++) res += "R";
        for(int i = 0; i < top && bot; i++) res += "D";
        for(int i = 0; i < bot && top; i++) res += "L";
        for(int i = 0; i < top && bot; i++) res += "U";
