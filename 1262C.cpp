...
void findAndReverse(vpii &res, string &s, int i, int k, char c) {
    int j = s.find_first_of(c, i + 1);
...
int main() {
    ...
    while(t--) {
        ...
        cout << sz(res) << endl;
        for(pii p : res) cout << p.f << " " << p.s << endl;
    }
}